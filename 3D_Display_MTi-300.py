...
# cil_course2 =
# cylinder(pos=(0.6,0,0),axis=(-0.2,0,0),radius=0.01,color=color.blue)
...
# the serial setting from the MTi-300
try:
    ser = serial.Serial()
    # ser.baudrate = 115200 # original
    ser.baudrate = 921600
    ser.port = 'COM3'
    ser.bytesize = 8
    ser.parity = 'N'
    # ser.stopbits = 2  # original
    ser.stopbits = 1
    ser.open()
    print ser
...
while True:
    try:
        (roll_degree, pitch_degree,
         Heading_degree) = getMtiEluerAngle.Get_MTi_Euler(ser)
        print roll_degree, pitch_degree, Heading_degree
        # put the data into the visual sence

        # transfer the degree to radian
        roll = roll_degree * grad2rad
        pitch = pitch_degree * grad2rad
        Heading = Heading_degree * grad2rad
        axis = (
            cos(pitch) * cos(Heading), -cos(pitch) * sin(Heading), sin(pitch))
        up = (sin(roll) * sin(Heading) + cos(roll) * sin(pitch) * cos(Heading),
        ...
        L1.text = str(roll_degree)
        L2.text = str(pitch_degree)
        L3.text = str(Heading_degree)
    except Exception, e:
        raise e
